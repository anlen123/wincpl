...
- 选中剪贴板条目后，在当前显示器右下角显示完整预览：文字可滚动，图片可切换适应窗口和原始尺寸。预览自动出现时不抢搜索焦点。
- 上下方向键选择，回车或鼠标点击粘贴。
...
拼音使用逐字默认读音，不包含声调、首字母简拼或多音词语境推断。标点和空格保留原边界；代码片段仍仅按原标题搜索。预览仅显示剪贴板文字和图片，随空结果、粘贴、关闭面板或切换到其他应用隐藏；点击预览可滚动，不会触发粘贴。
